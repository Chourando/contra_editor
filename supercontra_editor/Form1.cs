    ...
    public partial class Form1 : Form
    {
        void SplashScreen()
        {
            if (File.Exists("splash.png"))
            {
                ...
                splash.BackgroundImage = new Bitmap("splash.png");
                splash.Show();
        ...
        const string nesgamefile = "SuperContra.nes";
        ...
        Graphics pixel;
        Bitmap gamepic = new Bitmap("pal11.png");
        ...
        private void bt_loadmap_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("SuperContra.nes not found!", "Error",
        ...
        {
            StartReadBytes(nesgamefile + ".dat");
            tmpaddr = address;
            repeat = 0;
            tmp = 0;
            tmp2 = 0;
        ...
        {
            if (File.Exists("pal" + (int)data_ppu1.Value + (int)data_ppu2.Value + ".png"))
            {
                if (data_ppu1.Value != 2 || data_ppu1.Value != 4)
                {
                    gamepic = new Bitmap("pal" + (int)data_ppu1.Value + (int)data_ppu2.Value + ".png");
                    tmp = 0;
                    tmp2 = 0;
                    for (int i = 0; i < MAX_PICS; i++)
                    {
        ...
        private void bt_savemap_Click(object sender, EventArgs e)
        {
            if (File.Exists(nesgamefile))
            {
                if (File.Exists(nesgamefile + ".bak"))
                {
                    File.Delete(nesgamefile + ".bak");
                    File.Copy(nesgamefile, nesgamefile + ".bak");
                }
                else
                    File.Copy(nesgamefile, nesgamefile + ".bak");
                ReLoad(testaddress);
            ...
            {
                MessageBox.Show("SuperContra.nes not found!", "Info",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void bt_resetmap_Click(object sender, EventArgs e)
        {
            if (File.Exists(nesgamefile + ".bak"))
            {
                if (File.Exists(nesgamefile))
                {
                    ...
                    File.Copy(nesgamefile + ".bak", nesgamefile);
                }
                else
                    File.Copy(nesgamefile + ".bak", nesgamefile);
                ReLoad(testaddress);
        ...
        {
            if (File.Exists(nesgamefile + ".dat"))
            {
                File.Delete(nesgamefile);
                File.Copy(nesgamefile + ".dat", nesgamefile);
