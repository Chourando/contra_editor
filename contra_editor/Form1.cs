...
namespace contra_editor
{
    public partial class Form1 : Form
    {
        ...
        Graphics pixel;
        Bitmap gamepic = new Bitmap(workdir + "pal11.png");
        ...
        private void LoadPattern(int pid)
        {
            ...
            for (int i = 0; i < MAX_PICS; i++)
            {
                ...
                {
                    picnum[i, i2] = Convert.ToInt16(ReadBytes(tmpaddr, 1), 16);
                    tmpaddr += 0x1;
                }
            }
            CloseReadBytes();
        ...
        private void LoadEnemyData(int level)
        {
            ...
            {
                for (int i2 = 0; i2 < EDATA_LENGTH; i2++)
                {
                    edata[i, i2] = Convert.ToInt16(ReadBytes(tmpaddr, 1), 16);
                    tmpaddr += 0x1;
                }
            }
            CloseReadBytes();
        ...
        private void bt_loadmap_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Contra.nes not found!", "Error",
        ...
        private void ReLoad(uint address)
        {
            ...
            {
                data[i] = Convert.ToInt16(ReadBytes(tmpaddr, 1), 16);
        ...
        {
            StartReadBytes(workdir + nesgamefile + ".dat");
            tmpaddr = address;
            repeat = 0;
            tmp = 0;
            tmp2 = 0;
        ...
        {
            if (File.Exists(workdir + "pal" + (int)data_ppu1.Value + (int)data_ppu2.Value + ".png"))
            {
                if (data_ppu1.Value != 2 || data_ppu1.Value != 4)
                {
                    gamepic = new Bitmap(workdir + "pal" + (int)data_ppu1.Value + (int)data_ppu2.Value + ".png");
                    tmp = 0;
                    tmp2 = 0;
                    for (int i = 0; i < MAX_PICS; i++)
                    {
        ...
        private void bt_savemap_Click(object sender, EventArgs e)
        {
            if (File.Exists(nesgamefile))
            {
                if (File.Exists(workdir + nesgamefile + ".bak"))
                {
                    File.Delete(workdir + nesgamefile + ".bak");
                    File.Copy(workdir + nesgamefile, nesgamefile + ".bak");
                }
                else
                    File.Copy(nesgamefile, workdir + nesgamefile + ".bak");
                ReLoad(testaddress);
                LoadScene(hscb.Value, testaddress);
            ...
            {
                MessageBox.Show("Contra.nes not found!", "Info",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void bt_resetmap_Click(object sender, EventArgs e)
        {
            if (File.Exists(workdir + nesgamefile + ".bak"))
            {
                if (File.Exists(nesgamefile))
                {
                    ...
                    File.Copy(workdir + nesgamefile + ".bak", nesgamefile);
                }
                else
                    File.Copy(workdir + nesgamefile + ".bak", nesgamefile);
                ReLoad(testaddress);
        ...
        {
            if (File.Exists(workdir + nesgamefile + ".dat"))
            {
                File.Delete(nesgamefile);
                File.Copy(workdir + nesgamefile + ".dat", nesgamefile);
        ...
        string ReadBytes(uint address, int length)
        {
            br.BaseStream.Position = address;
            return BitConverter.ToString(br.ReadBytes(length));
        }
